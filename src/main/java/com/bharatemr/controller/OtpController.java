...
import com.bharatemr.dto.OtpResponseDto;
...
public class OtpController {
    ...
    @PostMapping("/verify")
    public ResponseEntity<ApiResponse<Object>> verifyOtp(@Valid @RequestBody OtpVerificationDto request) {
        ...
        if (purpose == OtpPurpose.LOGIN) {
            ...
            if ("DOCTOR".equals(request.getUserType())) {
                authResponse = doctorService.loginDoctor(request.getMobileNumber(), request.getOtp());
            } else {
                authResponse = patientService.loginPatient(request.getMobileNumber(), request.getOtp());
            }
